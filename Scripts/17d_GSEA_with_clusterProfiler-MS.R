...
resList <- resList[which(names(resList) %in% c("MS_Ramesh","MS_Schafflick"))]
...
ggsave("Results/Gene_set_enrichment_analysis/Gene_set_enrichment_dotplot_downsamp_MS.pdf", width = 8, height = 12)

gseTermSim <- pairwise_termsim(gse)
emapplot(gseTermSim, showCategory = 10)
ggsave("Results/Gene_set_enrichment_analysis/Emap_downsamp_MS.pdf", width = 8, height = 7)
