...
#############
#Initial steps, in principle. Due to github space restraint, only the slightly 
#processed files have been saved here. To regenerate the initial steps, please go to
#European Genome-phenome Archive—EGAD00001006232. 
#############
...
#Here, we also export a file with the B-lineage cell names, to be used for RNA velocity-associated
#cell cycle analyses. 
...
dir.create("Data/SingleCellExpFiles")
...
#These would of course be excluded. There are none, currently, though.
...
#It turns out that we in terms of specificity always need to separate the 
#LGI1s from the CASPR2s, so therefore, we introduce that change here. 
csfSce$Specific[which(csfSce$Specific == "TRUE")] <- "LGI1" 
csfSce$Specific[which(csfSce$Specific == "LGI1" &
                          csfSce$donor == "1284")] <- "CASPR2" 
table(csfSce$Specific, useNA = "ifany")
#    CASPR2      FALSE       LGI1 Not_tested 
#        33         27         85        322
