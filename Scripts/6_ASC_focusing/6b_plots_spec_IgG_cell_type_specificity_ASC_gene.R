...
#B cells 
bCsfSce <- csfSce[,-which(csfSce$clustersLouvain == "CD8T")]
set.seed(111)
bUmap <- uwot::umap(reducedDim(bCsfSce, "flowAndGLMPCA"))

dir.create("Results/Flow_specific")
flowDatLoc <- as.data.frame(t(normcounts(altExp(bCsfSce, "flowData")))[,c("CD38", "CD138", "CD27", "IgD", "CD20")])
...
bCsfSce$Specific[which(is.na(bCsfSce$Specific))] <- "Not_tested"
flowDatLoc$Specific <- factor(bCsfSce$Specific, levels = c("TRUE", "FALSE", "Not_tested"))
...
plot(flowDatLoc$IgD[which(bCsfSce$clustersLouvain == "B")],
     flowDatLoc$CD27[which(bCsfSce$clustersLouvain == "B")])

#When investigating these two plots, it is clear that the bulk of the CD27 positive
#cells are ASC. 
flowDatLoc$fineGrainedCellType <- "ASC"
flowDatLoc$fineGrainedCellType[which(bCsfSce$clustersLouvain == "B" & 
                                       flowDatLoc$CD27 > 2 &
...
igGDat <- flowDatLoc[which(colnames(bCsfSce) %in% BCR_IgG_test$CELL),]
...
bCsfSce$b_cell_type <- flowDatLoc$fineGrainedCellType
plotSce <- bCsfSce[which(rowData(bCsfSce)$hgnc_symbol %in% c("XBP1", "MS4A1")),]
