...
#Now, we will label all obvious doublets, which we will take out to become a separate
#cluster in the end. 
doublet <- rep(FALSE, nrow(depInData))
plot(depInData$CD3, depInData$CD138)
doublet[which((depInData$CD138 > 3.8 & depInData$CD3 > 4))] <- TRUE
plot(depInData$CD3, depInData$CD19)
abline(h = 4, v= 4)
doublet[which((depInData$CD19 > 4 & depInData$CD3 > 4))] <- TRUE
plot(depInData$CD56, depInData$CD19)
doublet[which((depInData$CD138 >2 & depInData$CD56 > 4))] <- TRUE
table(doublet)
#FALSE  TRUE 
#31987    28
#So this is a very gentle exclusion, but still important. We of course focus
#on the potential B cell doublets here. 
...
depModel <- depeche(depInData,
                    plotDir = "Diagnostics/Cytometry/Depeche")
...
clustersNamed <- sapply(depModel[[1]], switch, "1" = "CD4T", "2" = "NK",
                        "3" = "CD8T", "4" = "B", "5" = "ASC")
#And the doublet information is added
clustersNamed[which(doublet)] <- "doublet"
