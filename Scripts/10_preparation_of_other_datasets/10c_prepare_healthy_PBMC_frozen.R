...
singlerSce <- healthySceB[which(rowData(healthySceB)$ensembl_gene_id %in% row.names(ref)),]
refSce <- ref[which(row.names(ref) %in% rowData(healthySceB)$ensembl_gene_id),]
...
table(healthySceB$cellType, pred$pruned.labels)
#     ASC    B CD4T CD8T Myeloid
#ASC   35    0    0    0       0
#B     23 3754   78  129      22
healthySceB$cellTypeKotliarov <- pred$pruned.labels
healthySceBRefined <- healthySceB[,which(healthySceB$cellTypeKotliarov %in% c("ASC", "B"))]
healthySceBRefined <- healthySceBRefined[,-which(healthySceBRefined$cellTypeKotliarov == "ASC" &
                                                     healthySceBRefined$cellType != "ASC" )]
#table( healthySceBRefined$cellType,  healthySceBRefined$cellTypeKotliarov)
#     ASC    B
#ASC   35    0
#B      0 3754

saveRDS(healthySceBRefined, "../External/Data/Healthy/3_post_Kotliarov.rds")