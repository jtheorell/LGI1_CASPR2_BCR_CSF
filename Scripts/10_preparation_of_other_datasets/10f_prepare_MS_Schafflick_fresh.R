...
matrixNameList <- list.files("../External/Data/MS_Schafflick_CSF/GSE138266_CSF", pattern = "matrix", 
                         full.names = TRUE)
...
identical(fread("../External/Data/MS_Schafflick_CSF/GSE138266_CSF/GSM4104122_MS19270_CSF_GRCh38_genes.tsv.gz"),
          fread("../External/Data/MS_Schafflick_CSF/GSE138266_CSF/GSM4104124_MS71658_CSF_GRCh38_genes.tsv.gz"))
#TRUE

#So then we combine it all into one big dataframe after giving the cells some 
#colnames and then all get these rownames. 
fileNameList <- list.files("../External/Data/MS_Schafflick_CSF/GSE138266_CSF", pattern = "matrix")
...
rowDataMS <-  fread("../External/Data/MS_Schafflick_CSF/GSE138266_CSF/GSM4104122_MS19270_CSF_GRCh38_genes.tsv.gz",
                    header = FALSE)
...
saveRDS(msSchafSce, "../External/Data/MS_Schafflick_CSF/0_raw.rds")
...
dir.create("Diagnostics/Comp_to_others/MS_Schafflick_CSF", recursive = TRUE)
...
dev.copy(pdf,'Diagnostics/Comp_to_others/MS_Schafflick_CSF/Exclusion_metrics_per_donor_automatic.pdf', height = 5, width = 15)
dev.off()

msSchafSce<- msSchafSce[,-which(msSchafSce$discard)]
saveRDS(msSchafSce, "../External/Data/MS_Schafflick_CSF/1_lowQual_excluded.rds")
...
groupCode <- read.csv("../External/Data/MS_Schafflick_CSF/Group_code.csv")
...
dir.create("Data/Comp_to_others/MS_Schafflick_CSF/", recursive = TRUE)

saveRDS(pred, "Data/Comp_to_others/MS_Schafflick_CSF/MonacoSingler.rds")
#pred <- readRDS("Data/Comp_to_others/MS_Schafflick/MonacoSingler.rds")
unique(pred$pruned.labels)
#[1] "Th2 cells"                     "Non-Vd2 gd T cells"            "Myeloid dendritic cells"      
#[4] "Th1/Th17 cells"                "Intermediate monocytes"        "Terminal effector CD8 T cells"
#[7] "Th1 cells"                     "Non-switched memory B cells"   "Switched memory B cells"      
#[10] "MAIT cells"                    "Central memory CD8 T cells"    "Exhausted B cells"            
#[13] "Th17 cells"                    "Vd2 gd T cells"                "Plasmacytoid dendritic cells" 
#[16] "Terminal effector CD4 T cells" "Natural killer cells"          "T regulatory cells"           
#[19] NA                              "Classical monocytes"           "Plasmablasts"                 
#[22] "Naive CD4 T cells"             "Effector memory CD8 T cells"   "Follicular helper T cells"    
#[25] "Progenitor cells"              "Naive CD8 T cells"             "Naive B cells"                
#[28] "Non classical monocytes"    
...
saveRDS(msSchafSceB, "../External/Data/MS_Schafflick_CSF/2_normalised_post_monaco.rds")
...
singlerSce <- msSchafSceB[which(rowData(msSchafSceB)$ensembl_gene_id %in% row.names(ref)),]
refSce <- ref[which(row.names(ref) %in% rowData(msSchafSceB)$ensembl_gene_id),]
...
table(msSchafSceB$cellType, pred$pruned.labels)
#    ASC   B CD4T Precursor
#ASC 186   0    0         0
#B   172 425    8         3

msSchafSceB$cellTypeKotliarov <- pred$pruned.labels
msSchafSceBRefined <- msSchafSceB[,which(msSchafSceB$cellTypeKotliarov %in% c("ASC", "B"))]
msSchafSceBRefined <- msSchafSceBRefined[,-which(msSchafSceBRefined$cellTypeKotliarov == "ASC" &
                                           msSchafSceBRefined$cellType != "ASC" )]
table( msSchafSceBRefined$cellType,  msSchafSceBRefined$cellTypeKotliarov)
#    ASC   B
#ASC  38   0
#B     0 746

saveRDS(msSchafSceBRefined, "../External/Data/MS_Schafflick_CSF/3_post_Kotliarov.rds")
