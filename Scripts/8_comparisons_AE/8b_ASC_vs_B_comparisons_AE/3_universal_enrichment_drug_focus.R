...
#IFN-g HuZAF/unclear
#IGNGR unclear
#TNF  infliximab, adalimumab, etanercept, golimumab, and certolizumab
...
#Taken out, due to lack of clinically used drugs: "IL10", "IL10RA","IL10RB", "IFNG",
#"IFNGR1","IFNGR2"
...
resTab <- res$table
topGenes <- rowData(aeSce)$hgnc_symbol[which(resTab$logFC > 3 & resTab$PValue < 0.01)]
length(topGenes)
#284

#Here, the universe is defined as the genes expressed with a count of at least
#3 in 3 B lineage cells
aeSceB <- aeSce[,-which(aeSce$clustersLouvain == "CD8T")]
bCounts <- counts(aeSceB)
bExpressed <- sapply(seq_len(nrow(bCounts)), function(x){
    sum(bCounts[x,] > 2)
})

bCellUniverse <- unique(rowData(aeSce)$hgnc_symbol[which(bExpressed > 2)])
length(bCellUniverse)
#13291

#The drug tagets are now imported
drugTargetFileNameList <- list.files("Data/STRING_data", recursive = TRUE, full.names = TRUE)
...
#p-value = 0.03263, so this is significant. 

#So which are they?
topGenes[which(topGenes %in% names(drugTargetList))]
#"CD38" "SDC1" "IL6R"
...
fisherDf <- data.frame("Non_target" = c(length(nonTargUni)-nonTargetHit, nonTargetHit),
                       "Target" = c(length(unique(unlist(drugTargetList)))-targHit, targHit))
row.names(fisherDf) <- c("Non_hit", "Hit")
...
fisher.test(fisherDf, alternative = "greater")
#p-value = 0.5325
#So, no significance there. 

overRepRes <- lapply(drugTargetList, function(x){
    targHit <- length(which(topGenes %in% x))
    
    fisherDf <- data.frame("Non_target" = c(length(nonTargUni)-nonTargetHit, nonTargetHit),
                           "Target" = c(length(x)-targHit, targHit))
    row.names(fisherDf) <- c("Non_hit", "Hit")
    locPVal <- fisher.test(fisherDf, alternative = "greater")$p.value
    p.adjust(locPVal, length(drugTargetList), method = "fdr")
})

min(unlist(overRepRes))
#1

#I.e. none are significant. 

#All in all, this suggests that there is enrichment of the direct targets, specifically
#"CD38" "SDC1" "IL6R", but that there is no enrichment of the downstream molecules 
#for any of the targets. We of course do know that CD38 and CD138 would be enriched, 
#but IL6R is interesting. 
