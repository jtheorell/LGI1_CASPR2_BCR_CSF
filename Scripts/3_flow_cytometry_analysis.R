...
#And now, we normalize the peaks. We will do the FSC and SSC separately. 
normOrNot <- rep(TRUE, length(colnames(fsList[[1]])))
names(normOrNot) <- colnames(fsList[[1]])
normOrNot[c("TIME", "CD3")] <- FALSE
...
standFs <- flowSet(lapply(fsList[1:3], "[[", 4))
standFsTrans <- arcTrans(standFs, transNames = colnames(standFs)[5:16], 
                         transCoFacs = transCoFacs)

dir.create("Diagnostics/CytometryNorm", recursive = TRUE)
...
for(i in seq_len(ncol(subDf)-3)){
    ...
    ggsave(paste0("Diagnostics/CytometryNorm/", locName, "_preNorm.pdf"))
}

standFs <- flowSet(lapply(fsListTrans[1:3], "[[", 4))
...
for(i in seq_len(ncol(subDf)-3)){
    ...
    ggplot(locDf, aes(x=var, fill=acqDate)) +
        geom_density(alpha = 0.8) + xlab(locName) +ylab("")
    ggsave(paste0("Diagnostics/CytometryNorm/", locName, "_afterNorm.pdf"))
}
