...
flowDatLoc$Specific_2 <- csfSce$Specific
flowDatLoc$Specific_2[which(flowDatLoc$Specific == "TRUE")] <- "LGI1"
flowDatLoc$Specific_2[which(flowDatLoc$Specific == "TRUE" &
                                csfSce$donor == "1284")] <- "CASPR2"
flowDatLoc$Specific_2 <- factor(flowDatLoc$Specific_2, 
                                levels = c("FALSE", "LGI1", "CASPR2", "Not_tested"))
...
dColorPlot(as.character(bFlowReordered$Specific_2), colorScale = c("black", "orange", "#884444", "grey"),
           xYData = umapReordered, plotName = "Results/Figure_2_plots/Flow_specific/Specific_C_and_L", 
           densContour = FALSE)
dColorPlot(as.character(bFlowReordered$Specific_2)[-which(bFlowReordered$Specific == "Not_tested")], 
           colorScale = c("black", "orange", "#884444"),
           xYData = umapReordered[-which(bFlowReordered$Specific == "Not_tested"),], 
           dotSize = 20,
           plotName = "Results/Figure_2_plots/Flow_specific/Specific_spec_only_C_and_L", 
           densContour = FALSE)
...
spec_cell_df <- as.data.frame(table(as.character(igGDat$Specific_2), igGDat$cellType))
colnames(spec_cell_df) <- c("Specific_2", "Cell_type", "Freq")
spec_cell_df$B_subtype <- factor(spec_cell_df$Cell_type, c("ASC", "B"))
spec_cell_df$Specific_2 <- factor(spec_cell_df$Specific_2, c("FALSE",
                                                             "CASPR2",  
                                                             "LGI1"))
ggplot(spec_cell_df, aes(x=Specific_2, y=Freq, fill=Cell_type)) +
    geom_bar(stat="identity") + theme_bw() + 
    scale_y_continuous(limits = c(0,125),
                       expand = expansion(mult=c(0,0))) +
    scale_fill_manual(values = c("#2388DD","#BB3322"))
