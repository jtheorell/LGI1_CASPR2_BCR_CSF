...
colVals <- c("orange", "orange", "#FF6633")
...
lapply(donorSplit, function(x){
    ...
    p <- ggplot(netAll2, aes(x = x, y = y, xend = xend, yend = yend)) + 
        ...
        geom_edges() + scale_color_manual(values = c("black", "grey", x$colVal[1])) + theme_blank() +coord_equal()
    p
    ...
    p <- ggplot(netAll2, aes(x = x, y = y, xend = xend, yend = yend)) + 
        ...
        geom_edges() + scale_color_manual(values = c(x$colVal[1], "black", "grey", "yellow")) + theme_blank() +coord_equal()
    p
